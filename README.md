...
| Script | Purpose |
|---|---|
| `run_crow_server_in_foreground.sh` | Auto-builds if needed, checks port is free, runs the server with coloured output. `PORT=9090 ./run_crow_server_in_foreground.sh` to override the port. |
| `build_index_entry_for_frontend.sh` | GitHub Pages prep — builds the frontend with relative asset paths and copies `index.html` + `assets/` to the repo root. Not needed for local dev. |
